...
def wards(ward):
    ...
    sorted_data = sorted(filtered_data, key=lambda patient: int(patient["RoomNo"]))
    return render_template(
        "patients.html",
        data = sorted_data,
    )
...
def patient_details(patient_id):
    with open('data.json') as f:
        data = json.load(f)
    patient_data = next(patient for patient in data if patient["PatientID"] == patient_id)
    return render_template("details.html", patient=patient_data,
    )
